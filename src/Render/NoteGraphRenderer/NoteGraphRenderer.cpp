...
void NoteGraphRenderer::DrawNoteGraph(NoteGraph& graph, Area screenArea) {
	...
	float minDrawX = MAX(screenArea.min.x, graph.ToScreenPos(0, 0, screenArea.GetSize()).x);

	// Horizontal slot lines
	for (int i = 0; i < KEY_AMOUNT; i++) {
		auto screenY = screenArea.min.y + graph.ToScreenPos(0, i, screenArea.GetSize()).y;
	...
	for (int i = 0; i < 100; i++) {
		...
		auto screenX = screenArea.min.x + graph.ToScreenPos(t, 0, screenArea.GetSize()).x;
	...
	{ // Draw notes
		...
		for (Note* note : graph) {
			...
			Vec screenPos = screenArea.min + graph.ToScreenPos(note->time, note->key, screenArea.GetSize());
			float tailEndX = screenArea.min.x + graph.ToScreenPos(note->time + note->duration, 0, screenArea.GetSize()).x;
